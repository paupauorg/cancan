...
* Increase Performance (timraymond) (https://github.com/CanCanCommunity/cancan/pull/204)
...
* Complete cancan#115 - Specify authorization action for parent resources. (phallguy)

1.10.1 (January 13th, 2015)

* Fix cancan#168 - A bug with ActiveRecord 4.2 support causing ProtocolViolation due to named parameters not being passed in.
...
* Fix cancan#149 - Fix an issue loading namespaced models (darthjee)

* Fix cancan#160 - Support for Rails 4.2 (marshall-lee)

* Fix cancan#153 - More useful output in ability spec matchers (jondkinney)


1.9.2 (August 8th, 2014)

* Fix cancan#77, 78 - Fix an issue with associations for namespaced models. (jjp)


1.9.1 (July 21st, 2014)

* Fix cancan#101 - Fixes an issue where overjealous use of references would cause issues with scopes when loading associations. (bryanrite)


1.9.0 (July 20th, 2014)

* Fix cancan#59 - Parameters are automatically detected and santitized for all actions, not just create and update. (bryanrite)

* Fix cancan#97, 72, 40, 39, 26 - Support Active Record 4 properly with references on nested permissions. (scpike, tdg5, Crystark)


1.8.4 (June 24th, 2014)

* Fix cancan#86 - Fixes previous RSpec 3 update as there was a bug in the fix for RSpec 2.99. (bryanrite)


1.8.3 (June 24th, 2014)

* Fix cancan#85 - Remove deprecation notices for RSpec 3 and continue backwards compatibility. (andypike, bryanrite, porteta)


1.8.2 (June 5th, 2014)

* Fix cancan#75 - More specific hash-like object check. (bryanrite)


1.8.1 (May 27th, 2014)

* Fix cancan#67 - Sequel tests are run properly for JRuby. (bryanrite)

* Fix cancan#68 - Checks for hash-like objects in subject better. (bryanrite)
...
* Feature cancan#3 - Permit "can?" check multiple subjects (cefigueiredo)

* Feature cancan#29 - Add ability to use a String that will get instance_eval'd or a Proc that will get called as the parameter method option for strong_parameter santization (svoop)

* Feature cancan#48 - Define a CanCanCan module. Even though it is not used, it is standard practice to define the module, and helpful for determining between CanCanCan and CanCan for external libraries.
...
* Fix cancan#16 - RSpec expectations are not explicitly required in RSpec > 2.13 (justinaiken & bryanrite)
